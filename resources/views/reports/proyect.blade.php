...
<body>
    <div class="font">
        ...
        <div class="header-izq">
            <div><strong>Precision</strong></div>
        </div>
        ...
        <br>
        <br>
        <h3 class="center">Reporte Proyect</h3>
    </div>
    @if ($reporte_proyect)
        <p class="center font"><strong>Date: </strong>{{ $fecha_inicio }}<strong> to </strong>{{ $fecha_fin }}</p>
        @foreach ($reporte_proyect as $report)
            <table style=”width: 100%” class="font">
                <tr style="background: #cccccc">
                    <td style="text-align: start" colspan="5">
                        <strong>Id:</strong>{{ $report['Emp_ID'] }}&nbsp;&nbsp;
                        <strong>Name Company:</strong>{{ $report['Nombre'] }}&nbsp;&nbsp;&nbsp;
                        <strong>General Manager:</strong>{{ $report['Gerente_General'] }}&nbsp;&nbsp;
                ...
                <thead>
                    <tr>
                        <td rowspan="3"><strong>Num Proyect : </strong>{{ $report['Pro_ID'] }}</td>
                        <td colspan="4"><strong>Name Proyect: </strong>{{ $report['Nombre_proyecto'] }} </td>

                    </tr>
                    <tr>
                        <td><strong>General contractor: </strong>{{ $report['Contratista_General'] }}</td>
                        <td><strong>Status: </strong>{{ $report['estado'] }}</td>
                        <td><strong>Date start: </strong><br>{{ $report['Fecha_Inicio'] }}</td>
                        <td><strong>Date end: </strong><br>{{ $report['Fecha_Fin'] }}</td>
                    </tr>
                </thead>
                <tfoot>
                    <tr>
                        <td><strong>Zip Code: </strong>{{ $report['Zip_Code'] }}</td>
                        <td><strong>Street: </strong>{{ $report['Calle'] }}</td>
                        <td><strong>Status: </strong><br>{{ $report['estado'] }}</td>
                        <td><strong>Hours: </strong><br>{{ $report['Horas'] }}</td>
                    </tr>
                </tfoot>
                <tbody class="total">
                    @if ($report['actividades'])
                        ...
                        <tr>
                            ...
                            <th><strong>Foreman_name</strong></th>
                            <th><strong>Hours</strong></th>
                            <th><strong>Status</strong></th>
                        </tr>
                        @foreach ($report['actividades'] as $actividad)
                            <tr>
                    ...
                    @else
                        <tr>
                            <td colspan="5">there is no associated task</td>
                        </tr>
                    @endif
                </tbody>
            </table>
        ...
        @endforeach
    @else
        <p class="center font" style="text-align: center">no registered task from {{ $fecha_inicio }} to
            {{ $fecha_fin }}
        </p>
    @endif
