<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>report of Tasks</title>
</head>
<style>
    .font {
        font-family: Arial, Helvetica, sans-serif;
        font-size: 12px;
    }

    .header-izq {
        float: left;
    }

    .header-der {
        float: right;
    }

    .center {
        text-align: center;
    }

    .container {
        background: #b4b3b3;
        margin: 0 0 1rem;
        height: 10rem;
        display: flex;
        /* align-items por defecto tiene el valor `stretch` */
        align-items: start;
    }

    table,
    td,
    th {
        border: 1px solid #ddd;
        text-align: center;
    }

    table {
        border-collapse: collapse;
        width: 100%;
    }

    th,
    td {
        padding: 3px;
    }

    @page {
        margin: 65px 50px;
    }

    footer .page:after {
        content: counter(page);

    }

    .total {
        background: #f5f3f3
    }

    .corte-control {
        float: left;
        margin-left: 5px;
    }
</style>

<body>
    <div class="font">
        <div class="header-izq">
            <img
                src="data:image/jpeg;base64,">
        </div>
        <div class="header-izq">
            <div><strong>Precision</strong></div>
        </div>

        <div class="header-der">
            <div><strong>Report date: </strong>{{ date('Y-m-d') }}</div>
        </div>
        <br>
        <br>
        <h3 class="center">Reporte Tasks</h3>
    </div>
    @if ($reporte_task)
        <p class="center font"><strong>Date: </strong>{{ $fecha_inicio }}<strong> to </strong>{{ $fecha_fin }}</p>
        @foreach ($reporte_task as $report)
            <table style=”width: 100%” class="font">
                <tr style="background: #cccccc">
                    <td style="text-align: start" colspan="5">
                        <strong>Id:</strong>{{ $report['Pro_ID'] }}&nbsp;&nbsp;
                        <strong>Name Proyect:</strong>{{ $report['Nombre'] }}&nbsp;&nbsp;&nbsp;
                        <strong>Status:</strong>{{ $report['estado'] }}&nbsp;&nbsp;
                        <strong>Code Company:</strong>{{ $report['Codigo'] }}&nbsp;&nbsp;&nbsp;
                    </td>
                </tr>
                @if ($report['actividades'])
                    @foreach ($report['actividades'] as $actividad)
                        <colgroup>
                            <col style="width: 20%" />
                            <col style="width: 40%" />
                            <col style="width: 40%" />
                        </colgroup>
                        <thead>
                            <tr>
                                <td rowspan="3"><strong>Num Task : </strong>{{ $actividad['Actividad_ID'] }}</td>
                                <td colspan="4"><strong>name Task: </strong>{{ $actividad['Actividad_Nombre'] }} </td>

                            </tr>
                            <tr>
                                <td><strong>Status: </strong>{{ $actividad['Estatus'] }}</td>
                                <td><strong>Time: </strong>{{ $actividad['Hora'] }}</td>
                                <td><strong>Date: </strong><br>{{ $actividad['Fecha'] }}</td>
                                <td><strong>Start date: </strong><br>sdsdsd</td>
                            </tr>
                        </thead>
                        <tfoot>
                            <tr>
                                <td colspan="4"><strong>Description :</strong>{{ $actividad['Descripcion'] }}</td>
                            </tr>
                        </tfoot>
                        <tbody class="total">
                            @if ($actividad['tickets'])

                                <tr>
                                    <th><strong>Id</strong></th>
                                    <th><strong>Number</strong></th>
                                    <th><strong>Foreman_name</strong></th>
                                    <th><strong>Hours</strong></th>
                                    <th><strong>Status</strong></th>
                                </tr>
                                @foreach ($actividad['tickets'] as $ticket)
                                    <tr>
                                        <td>{{ $ticket->ticket_id }}</td>
                                        <td>{{ $ticket->num }}</td>
                                        <td>{{ $ticket->foreman_name }}</td>
                                        <td>{{ $ticket->horario }}</td>
                                        <td>{{ $ticket->estado }}</td>
                                    </tr>
                                @endforeach
                            @else
                                <tr>
                                    <td colspan="5">there is no associated tickets</td>
                                </tr>
                            @endif
                        </tbody>
                    @endforeach
                @else
                    <thead>
                        <td>there is no registered task</td>
                    </thead>
                @endif
            </table>
            <br>
        @endforeach
    @else
        <p class="center font" style="text-align: center">no registered task from {{ $fecha_inicio }} to
            {{ $fecha_fin }}
        </p>
    @endif

</body>
<!--footer>
    <p class="header-der page">
        Página
    </p>
</footer-->

</html>
