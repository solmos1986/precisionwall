...
$(document).on("click", ".view_material", function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            //select_material();
            $('#view_superficie_id').val(response.data.superficie_id);
            console.log($('#view_superficie_id').val())
            lista(response.data.materiales);
            select_material();
            //view standares
            view_estandares(response.data.estandares)
            $('#modalViewMateriales').modal('show');
        }
    });
});
function view_estandares(estandares) {
    $('#view_standares').html('');
    var estandaresHTML = ``;
    estandares.forEach(estandar => {
        estandaresHTML += `
    <li role="presentation" class="fs-12 mb-2"> <a href="#btc" aria-controls="btc" aria-selected="false">
        ${estandar.codigo} | ${estandar.nombre}
    </a></li>
    `;
    });
    $('#view_standares').append(estandaresHTML)
...
function lista(materiales) {
    ...
    materiales.forEach(material => {
        ...
        <tr>
            ...
            <td >${material.Unidad_Medida== null ? '' : material.Unidad_Medida}</td>
            <th >
